...
class LoginPage:
    def __init__ (self,top):
        ...
        self.top.configure(bg="#6d6f8e")
        self.top.title("Login Page")
        self.title_label = Label(self.top, text="Restaurant management system", font=('arial', 35, 'bold'), bg="#926c90")
        self.title_label.pack(side=TOP, fill=X)

        self.main_frame = Frame(self.top, bg="#9785a7")
        self.main_frame.place(x=450, y=150,width=600,height=350)


        self.login_lb = Label(self.main_frame,text="Login",font=('arial',30,'bold'),bg="#9785a7")
        self.login_lb.place(x=270,y=10)

        username = StringVar()
        password = StringVar()

        self.user_lb = Label(self.main_frame,text="Username: ",font=('arial',13,'bold'),bg="#9785a7",fg="black")
        self.user_lb.place(x=100, y=100)

        self.user_entry = Entry(self.main_frame, font=("arial",15),bg="#e6dcd1",relief=GROOVE, textvariable=username)
        self.user_entry.place(x=200 ,y=100 )

        self.pwd_lb = Label(self.main_frame, text="Password: ", font=('arial',13,'bold'), bg="#9785a7",fg="black")
        self.pwd_lb.place(x=100, y=150)

        self.pwd_entry = Entry(self.main_frame, font=("arial",15),bg="#e6dcd1",relief=GROOVE, textvariable=password,show="* ")
        ...
        self.login_btn = Button(self.main_frame,text="Login",font=('arial',13,'bold'), bg="#9785a7",fg="black",command=check_login)
        self.login_btn.place(x=230,y=200)

        self.sign_up_btn = Button(self.main_frame,text="signup",font=('arial',13,'bold'), bg="#9785a7",fg="black",command=sign_up)
        self.sign_up_btn.place(x=300,y=200)

        self.reset_btn = Button(self.main_frame,text="Reset",font=('arial',13,'bold'), bg="#9785a7",fg="black",command=reset)
...
class BillingPage:
    def __init__(self,top):
        ...
        self.entry_frame = LabelFrame(self.top,text="Details",bg="#9785a7",font=('Arial',20),bd=7,relief=GROOVE)
        self.entry_frame.place(x=20, y=95, width=550, height=650)

        self.bill_no_lb = Label(self.entry_frame,text="Bill Number: ",font=('arial',13,'bold'),bg="#9785a7",fg="black")
        self.bill_no_lb.grid(row=0, column=0,padx=2,pady=2)

        self.bill_no_entry = Entry(self.entry_frame,bd=5,textvariable=bill_no_tk, bg="#e6dcd1",font=('Arial',15))
        self.bill_no_entry.grid(row=0,column=1,padx=2,pady=2)
        self.bill_no_entry.config(state=DISABLED)

        self.Custumer_name_lb = Label(self.entry_frame, text="Customer Name: ", font=('arial',13,'bold'),bg="#9785a7",fg="black")
        self.Custumer_name_lb.grid(row=1, column=0, padx=2, pady=2)

        self.Custumer_name_entry = Entry(self.entry_frame, bd=5, textvariable=cust_nm, bg="#e6dcd1",font=('Arial', 15))
        self.Custumer_name_entry.grid(row=1, column=1, padx=2, pady=2)

        self.Custumer_conct_lb = Label(self.entry_frame, text="Customer Contact: ", font=('arial',13,'bold'),bg="#9785a7",fg="black")
        self.Custumer_conct_lb.grid(row=2, column=0, padx=2, pady=2)

        self.Custumer_conct_entry = Entry(self.entry_frame, bd=5, textvariable=cust_conct, bg="#e6dcd1",font=('Arial', 15))
        self.Custumer_conct_entry.grid(row=2, column=1, padx=2, pady=2)

        self.Date_lb = Label(self.entry_frame, text="Date: ", font=('arial',13,'bold'),bg="#9785a7",fg="black")
        self.Date_lb.grid(row=3, column=0, padx=2, pady=2)

        self.Date_entry = Entry(self.entry_frame, bd=5, textvariable=date_pr, bg="#e6dcd1",font=('Arial', 15))
        self.Date_entry.grid(row=3, column=1, padx=2, pady=2)
        self.Date_entry.config(state=DISABLED)


        self.item_pur_lb = Label(self.entry_frame, text="Item name: ", font=('arial',13,'bold'),bg="#9785a7",fg="black")
        self.item_pur_lb.grid(row=4, column=0, padx=2, pady=2)

        self.item_pur_entry = Entry(self.entry_frame, bd=5, textvariable=item_pur, bg="#e6dcd1",font=('Arial', 15))
        self.item_pur_entry.grid(row=4, column=1, padx=2, pady=2)

        self.item_qunt_lb = Label(self.entry_frame, text="Item Quantity: ", font=('arial',13,'bold'),bg="#9785a7",fg="black")
        self.item_qunt_lb.grid(row=5, column=0, padx=2, pady=2)

        self.item_qunt_entry = Entry(self.entry_frame, bd=5, textvariable=item_qty, bg="#e6dcd1",font=('Arial', 15))
        self.item_qunt_entry.grid(row=5, column=1, padx=2, pady=2)

        self.cost_lb = Label(self.entry_frame, text="Cost of One: ", font=('arial',13,'bold'),bg="#9785a7",fg="black")
        self.cost_lb.grid(row=6, column=0, padx=2, pady=2)

        self.cost_entry = Entry(self.entry_frame, bd=5, textvariable=cost_of_one, bg="#e6dcd1",font=('Arial', 15))
        ...
        def total_func():
            ...
            self.grd_total += self.grd_total * 0.12
            self.bill_txt.insert(END,"\n=================================================================================")
            self.bill_txt.insert(END,f"\t\t\t\t\t\t              Total : {self.grd_total} (With GST)")
            self.bill_txt.insert(END,"\n=================================================================================")
        ...
        self.button_frame = LabelFrame(self.entry_frame,bd=5,text="Option",bg="#9785a7",font=('Arial',15))
        self.button_frame.place(x=20, y=280, width= 390, height=300)

        self.add_btn = Button(self.button_frame, bd=2,text="Add",bg="#9785a7",fg="black",width=12, height=3,command=add_func)
        self.add_btn.grid(row=0, column=0, padx=4,pady=2)

        self.generate_btn = Button(self.button_frame, bd=2,text="Generate",bg="#9785a7",fg="black",width=12, height=3,command=genbill)
        self.generate_btn.grid(row=0, column=1, padx=4,pady=2)

        self.clear_btn = Button(self.button_frame, bd=2,text="Clear",bg="#9785a7",fg="black",width=12, height=3,command=clear_func)
        self.clear_btn.grid(row=0, column=2, padx=4,pady=2)

        self.total_btn = Button(self.button_frame, bd=2, text="Total", bg="#9785a7",fg="black", width=12, height=3,command=total_func)
        self.total_btn.grid(row=1, column=0, padx=4, pady=2)

        self.reset_btn = Button(self.button_frame, bd=2, text="Reset", bg="#9785a7",fg="black", width=12, height=3,command=reset_func)
        self.reset_btn.grid(row=1, column=1, padx=4, pady=2)
        
        self.save_btn = Button(self.button_frame, bd=2, text="Save", bg="#9785a7",fg="black", width=12, height=3,command=save_fucn)
        ...
        self.cal_frame = Frame(self.top,bd=8,bg="#9785a7",relief=GROOVE)
        self.cal_frame.place(x=650,y=90,width=720,height=315)

        self.num_entry = Entry(self.cal_frame,bd=15,bg="#e6dcd1",textvariable=cal_var,font=('Arial',15),width=60,justify='right')
        ...
        self.cal_btn1 = Button(self.cal_frame,bg="#9785a7",fg="black",text="1",bd=8,width=15,height=2)
        self.cal_btn1.grid(row=1,column=0, padx=2, pady=2)
        self.cal_btn1.bind("<Button-1>",press_btn)

        self.cal_btn2 = Button(self.cal_frame, bg="#9785a7",fg="black", text="2", bd=8, width=15, height=2)
        self.cal_btn2.grid(row=1, column=1, padx=2, pady=2)
        self.cal_btn2.bind("<Button-1>", press_btn)

        self.cal_btn3 = Button(self.cal_frame, bg="#9785a7",fg="black", text="3", bd=8, width=15, height=2)
        self.cal_btn3.grid(row=1, column=2, padx=2, pady=2)
        self.cal_btn3.bind("<Button-1>", press_btn)

        self.cal_btn_add = Button(self.cal_frame, bg="#9785a7",fg="black", text="+", bd=8, width=15, height=2)
        self.cal_btn_add.grid(row=1, column=3, padx=2, pady=2)
        self.cal_btn_add.bind("<Button-1>", press_btn)

        self.cal_btn4 = Button(self.cal_frame, bg="#9785a7",fg="black", text="4", bd=8, width=15, height=2)
        self.cal_btn4.grid(row=2, column=0, padx=2, pady=2)
        self.cal_btn4.bind("<Button-1>", press_btn)

        self.cal_btn5 = Button(self.cal_frame, bg="#9785a7",fg="black", text="5", bd=8, width=15, height=2)
        self.cal_btn5.grid(row=2, column=1, padx=2, pady=2)
        self.cal_btn5.bind("<Button-1>", press_btn)

        self.cal_btn6 = Button(self.cal_frame,bg="#9785a7",fg="black", text="6", bd=8, width=15, height=2)
        self.cal_btn6.grid(row=2, column=2, padx=2, pady=2)
        self.cal_btn6.bind("<Button-1>", press_btn)

        self.cal_btn_sub = Button(self.cal_frame, bg="#9785a7",fg="black", text="-", bd=8, width=15, height=2)
        self.cal_btn_sub.grid(row=2, column=3, padx=2, pady=2)
        self.cal_btn_sub.bind("<Button-1>", press_btn)

        self.cal_btn7 = Button(self.cal_frame, bg="#9785a7",fg="black", text="7", bd=8, width=15, height=2)
        self.cal_btn7.grid(row=3, column=0, padx=2, pady=2)
        self.cal_btn7.bind("<Button-1>", press_btn)

        self.cal_btn8 = Button(self.cal_frame, bg="#9785a7",fg="black", text="8", bd=8, width=15, height=2)
        self.cal_btn8.grid(row=3, column=1, padx=2, pady=2)
        self.cal_btn8.bind("<Button-1>", press_btn)

        self.cal_btn9 = Button(self.cal_frame, bg="#9785a7",fg="black", text="9", bd=8, width=15, height=2)
        self.cal_btn9.grid(row=3, column=2, padx=2, pady=2)
        self.cal_btn9.bind("<Button-1>", press_btn)

        self.cal_btn_mul = Button(self.cal_frame, bg="#9785a7",fg="black", text="*", bd=8, width=15, height=2)
        self.cal_btn_mul.grid(row=3, column=3, padx=2, pady=2)
        self.cal_btn_mul.bind("<Button-1>", press_btn)

        self.cal_btn0 = Button(self.cal_frame, bg="#9785a7",fg="black", text="0", bd=8, width=15, height=2)
        self.cal_btn0.grid(row=4, column=0, padx=2, pady=2)
        self.cal_btn0.bind("<Button-1>", press_btn)

        self.cal_btn_dot = Button(self.cal_frame, bg="#9785a7",fg="black", text=".", bd=8, width=15, height=2)
        self.cal_btn_dot.grid(row=4, column=1, padx=2, pady=2)
        self.cal_btn_dot.bind("<Button-1>", press_btn)

        self.cal_btn_eql = Button(self.cal_frame, bg="#9785a7",fg="black", text="=", bd=8, width=15, height=2)
        self.cal_btn_eql.grid(row=4, column=2, padx=2, pady=2)
        self.cal_btn_eql.bind("<Button-1>", press_btn)

        self.cal_btn_div = Button(self.cal_frame, bg="#9785a7",fg="black", text="/", bd=8, width=15, height=2)
        self.cal_btn_div.grid(row=4, column=3, padx=2, pady=2)
        self.cal_btn_div.bind("<Button-1>", press_btn)

        self.bill_frame = LabelFrame(self.top,text="Bill Area",font=('Arial',20),bg="#9785a7",bd=8,relief=GROOVE)
        self.bill_frame.place(x=685, y=430, width=690,height=330 )

        self.y_scroll = Scrollbar(self.bill_frame,orient="vertical")
        self.bill_txt = Text(self.bill_frame,bg="#e6dcd1",yscrollcommand=self.y_scroll.set)
